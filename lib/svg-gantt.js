...
class SvgGantt {
  ...
  constructor(rows, columns) {
    ...
    this.nodes.svg.svg[0].g.g = rows.map((r, i) =>
      this._titleNode(
        SvgGantt.COLUMN_WIDTH * columns.length - SvgGantt.COLUMN_WIDTH / 2,
        i * SvgGantt.ROW_HEIGHT,
        `${r.meta.name} ${r.cycle}`
      )
    );
    this.nodes.svg.svg[1].g[0].g = columns.map((c, i) =>
      this._headerNode(
    ...
    this.nodes.svg.svg[1].g[0].g.push(this._timeLineNode(rows, columns));
    ...
    this.nodes.svg.svg[1].g[3].g = rows.flatMap(({ meta, release, support, eol }, i) => {
  ...
  _timeLineNode(rows, columns) {
    ...
    return {
      ...
      text: {
        ...
        '#text': `${month(now.getUTCMonth())} ${now.getUTCDate()} ${now.getUTCFullYear()}`,
      },
    };
  }
  ...
  _barPosition(columns, start, end) {
    ...
    const offset = SvgGantt.TITLE_WIDTH + SvgGantt.COLUMN_WIDTH * (si + (start - columns[si]) / (columns[si + 1] - columns[si]));
    const distance = SvgGantt.TITLE_WIDTH + SvgGantt.COLUMN_WIDTH * (ei + (end - columns[ei]) / (columns[ei + 1] - columns[ei]));
