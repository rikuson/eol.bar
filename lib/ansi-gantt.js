...
class AnsiGantt {
  ...
  constructor(rows, columns) {
    ...
    const list = rows.map(({ meta, cycle, release, support, eol }) => {
      let line = '';
      line += this._textRight(`${meta.name} ${cycle}`, AnsiGantt.COLUMN_OFFSET);
      const active = this._barPosition(columns, release, support ? support : eol);
      line += separator.slice(
        AnsiGantt.COLUMN_OFFSET,
        AnsiGantt.COLUMN_OFFSET + Math.floor(active.offsetX)
      );
  ...
  _header(columns) {
    const offsetX = join(range(AnsiGantt.COLUMN_OFFSET, ' '));
  ...
  _separator(columns) {
    const offsetX = join(range(AnsiGantt.COLUMN_OFFSET - AnsiGantt.COLUMN_WIDTH / 2 - 1, ' '));
